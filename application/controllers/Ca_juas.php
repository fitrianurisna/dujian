...
class Ca_juas extends CI_Controller
{
	...
	public function dh_uas_pdf($id = '')
	{
		$data["jadwalt"] = $this->M_juas->Get($id);
		$data['jadwalk'] = $this->M_juas->get_jadwal_where($id)->row_array();
		$data['jadwalq'] = $this->M_juas->getmahasiswa($data['jadwalk']['matkul'])->result_array();
  ...
  // //       echo "<pre>";
		...
		$this->pdf->filename = "Daftar Hadir Susulan UAS.pdf";
		$this->pdf->load_view('admin/dh_uas_pdf', $data);
	}
